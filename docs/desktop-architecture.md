...
Canvas shapes must reference heavy assets by ID. Ports and workflow arrows are hidden during normal creation and become visible and interactive only in Flow Edit mode. Dragging the enlarged right output target onto a left input target shows a live connection preview and creates an exact, center-anchored curved tldraw arrow binding; click-output then click-input remains a fallback. Existing card connections are normalized on load. These bindings are compiled into `project_graphs` before each run, and freeform visual position is never an execution dependency. Generation-to-generation edges pass output asset references, image URLs, and seeds into downstream `img2img` parameters.

The normal canvas renders generation work as lightweight image result cards containing the image, status, ratio/model summary, and prompt summary. Prompt entry, model selection, and ratio selection live in the bottom composer; advanced parameters remain in the contextual inspector. Double-clicking a result opens that inspector. Store listeners persist both user gestures and programmatic updates, including workflow arrows, parameters, status changes, and generated results.

The global generation entry is a single-row composer at the bottom of the canvas. It creates and immediately runs a root generation when no result is selected, or creates an iteration from the selected result. Workflow relationships remain persisted but are rendered only in Flow Edit mode. Execution orchestration belongs to the workspace so the bottom composer and right-panel run controls share one state machine. The right panel defaults to closed; when opened, run history is folded and the advanced inspector remains contextual.
